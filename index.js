...
//
const BEHAVIORS = [
  { name: "lookAround",   weight: 30 },  // most common — zero movement flags
  { name: "microStep",    weight: 22 },  // pathfinder = valid physics, no SurvivalFly
  { name: "sneakBrief",   weight: 16 },  // sneak toggle — common human behaviour
  { name: "hotbarSwitch", weight: 14 },  // slot switch — NCP doesn't flag this
  { name: "swingArm",     weight: 10 },  // arm swing packet — totally safe
  { name: "jumpOnce",     weight:  8 },  // rare jump — NCP watches jump freq
];
  ...
  try {
    switch (name) {
      ...
      case "sneakBrief": {
        ...
        await new Promise(r => setTimeout(r, rand(400, 2000)));
      ...
      case "jumpOnce": {
        // ONLY jump when fully stationary — NCP SurvivalFly flags jump+move combos
        if (typeof bot.setControlState !== "function") break;
        bot.pathfinder.setGoal(null); // make sure not moving
